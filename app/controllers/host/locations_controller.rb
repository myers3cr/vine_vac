class Host::LocationsController < ApplicationController
  ...
  def create
    ...
    authorize @location
    if @location.save
      redirect_to host_locations_path, notice: "Location added"
    else
      render :new
    end
  end
  ...
  def update
    ...
    if @location.update(location_params)
      ...
      redirect_to host_locations_path, notice: 'Location updated.'
    else
        render :edit
    end
  end
  ...
  def destroy
    @location.destroy
    redirect_to host_locations_path, notice: 'Location deleted.'
  end
  ...
  def location_params
    params.require(:location).permit(:member_id, :loc_type, :nearest_lake,:name, :description,
      :address_1, :address_2, :city, :state, :postal_code, :images, :occupancy)
  end
